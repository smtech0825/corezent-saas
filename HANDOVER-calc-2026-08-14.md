...
**S5 잔여(재산세·종부세 — 2026-08-14 Wave 6 점검분, 수정 보류 지시)**
Medium 2건: ① 과세기준일 룰(property/comprehensive.assessment_date)을 연중 시행일로 등록하면 연초(1월 1일) 임시 조회 단계에서 "룰 미등록"으로 실패하는데 오류 메시지가 그 사정을 설명하지 못함(가이드 경고로 완화 — 전용 메시지 권장). ② 종부세 기준일 배너가 공제 계산에 쓰이는 재산세 룰을 집계하지 않음(taxTypes=['comprehensive']만) — 과대 집계 방지를 위해 승인된 설계이나, 재산세 룰만 갱신되면 배너 날짜가 움직이지 않는 한계 인지(실제 쓰인 재산세 룰은 결과 근거 목록에 표시됨).
Low 8건(요지): 공시가격 합계가 기본공제를 수십 원 차로 넘으면 종부세 과세표준 0 절사 → 재산세 엔진의 맥락 없는 오류 문구("공시가격은 0보다 커야") · 종부세 계산 과정 표시에서 재산세 공제가 산출세액을 넘는 경우 뺄셈 표기 불일치 가능(rawTax 절사 시점 포함) · 직전 연도 값 극소 양수(1원 등)는 상한이 낮게 걸림 — 명시 입력이라 의도 동작으로 판단 · 나이(65)·연수(7) placeholder가 요건 경계로 오해될 소지(양도세 전례상 허용) · 주택 수 1/2/3 고정 모델(양도세 관례 동일 — 룰이 다른 구분선 요구 시 표현 불가) · 재산세 폼 도시지역 체크가 고급 접힘 후에도 값 유지(결과 패널이 제외 표시로 보완) · 종부세 세부담 상한 비교 기준 비대칭(직전=총세액 vs 당해=상당액+종부세 — 승인 산식, 세액 낮추는 방향) · property.ts 305줄(300줄 근소 초과 — 승인 예외).

**S6 잔여(등기비용·매도 실수령액 — 2026-08-14 Wave 6 점검분, 수정 보류 지시)**
Medium 6건: ① 채권 손실률 미입력이어도 bond 룰 조회를 먼저 수행 — 룰 미등록이면 전체 중단(필수 룰 대원칙 부합 측면 있어 의도 여부 판단 필요). ② 등기비용의 인지세를 취득일 기준으로 계산(계약일 입력 없음) — 계약일·잔금일 사이 세액표 개정 시 인지세 계산기와 상이 가능. ③ 실수령액 폼 필요경비 힌트에 '중개수수료' 누락(양도세 폼과 문구 불일치 — 힌트대로면 세액 과대. 검증·전송 필드는 완전 일치). ④ 채권 행이 is_metro 조건일 때 metro_scope 룰이 근거 목록 누락(양도세 엔진도 동일 전례). ⑤ 등기비용 패널 unresolvedFields 영문 노출(취득세 병합분 — 한국어 라벨 맵 필요). ⑥ 실수령액 '그 밖의 비용' 미입력 시 0 간주+결과 행 생략(미포함 표시 없음 — 등기비용 not_included 설계와 불일치). +불확실 1: 인지세 전액 매수인 부담 합산 전제가 안내에 없음(세법 해석 필요).
Low 요지: 경과조치 날짜 함수 복제(transfer/net-proceeds page) · netProceeds 필드명이 양도세 breakdown과 정의 다른데 동명 · transfer 결과 통째 전달로 appliedRules payload 중복 · 등기 폼 주택 수 정수 미검증 · 실수령액 큰 숫자 산식 캡션 없음 · text-[11px]·아이콘 직접 import·aria-hidden·OG 이미지(전역 관례·과제) · 중개수수료 기준일=양도일(계약 시점 아님 — 갈음 안내로 완화).
