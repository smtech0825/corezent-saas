...
│   │   ├── faq/               # FAQ
│   │   ├── security/          # ⭐신규 보안·개인정보 안내(신뢰 센터) — DB 미조회 정적 콘텐츠, 문구 근거는 지니워크 마스터 문서
...
| 회원 기관 정보·세금계산서 ⭐신규 | [src/lib/bizRegNo.ts](src/lib/bizRegNo.ts) — `normalizeBizRegNo`·`isValidBizRegNo`(국세청 체크디짓)·`formatBizRegNo` 단일 출처. 입력: [dashboard/settings/OrgProfileSection.tsx](src/app/dashboard/settings/OrgProfileSection.tsx) / 조회: [admin/users/[id]](src/app/admin/users/[id]/page.tsx) 읽기 전용 구역. 저장: `profiles.org_name`·`org_biz_reg_no`(숫자 10자리)·`org_contact_name`·`tax_invoice_email`(**마이그레이션 068** — 운영자 수동 적용). ⚠️ **`profiles.org_*`(회원이 저장한 기본값)와 `orders.org_*`(061, 그 주문의 확정값)는 별개** — 자동으로 옮겨가지 않는다. ⚠️ 관리자 상세에서는 반드시 **별도 조회**로 읽는다 — 기존 profile select에 합치면 068 미적용 시 조회 실패로 사용자 상세가 404가 된다. 068 미적용이면 양쪽 모두 구역만 숨긴다 |
| 설치파일 체크섬(SHA-256) ⭐신규 | [src/lib/checksum.ts](src/lib/checksum.ts) — `normalizeChecksum`(`sha256:` 접두어·공백 제거, 소문자화)·`SHA256_RE` 단일 출처. 편집기와 서버 액션이 같은 규칙을 써야 화면 통과/저장 거부가 갈리지 않는다. [src/lib/platforms.ts](src/lib/platforms.ts) — 플랫폼 키·표시 이름 단일 출처(기존 3곳 중복 통합). [src/components/common/ChecksumList.tsx](src/components/common/ChecksumList.tsx) — 표시 공용(대시보드 라이선스·공개 변경 이력). ⚠️ **다운로드 주소가 없는 플랫폼의 체크섬은 저장·표시하지 않는다** — 어느 파일 해시인지 알 수 없는 값이 남는다. 저장 위치는 기존 `changelogs.checksums`(006, 마이그레이션 불필요) |
| 보안·개인정보 안내 ⭐신규 | [src/app/security/page.tsx](src/app/security/page.tsx) — 기관 보안 검토용 신뢰 센터. ⚠️ 「외부 전송 없음」 금지 — **당사 서버 무전송**과 **AI 회사로 가린 글자 전송**을 반드시 나눠 적는다. 가리지 않는 항목(주소·금액·이름·계좌)까지 밝히는 것이 이 페이지의 요점. 근거는 `prompts/지니워크_제품정보_마스터.md` §3-4·§5와 [/api/license/validate](src/app/api/license/validate/route.ts)가 실제로 받는 필드(`key`·`hwid`·`product`뿐). 진입: 푸터 '자료' 그룹 · [/public-sector](src/app/public-sector/page.tsx) 보안 섹션 말미(자세한 설명은 이쪽 한 곳에만 둔다) |
| 소셜 미리보기 이미지(og:image) ⭐신규 | [src/lib/og-image.tsx](src/lib/og-image.tsx) — `renderOgImage({title, subtitle})` 단일 출처(1200×630, 페이퍼 톤). 적용: [app/opengraph-image.tsx](src/app/opengraph-image.tsx)(기본, 하위 전 페이지 상속) · [security](src/app/security/opengraph-image.tsx) · [public-sector](src/app/public-sector/opengraph-image.tsx). 한글은 견적서와 같은 나눔고딕(OFL) TTF를 fs로 읽는다. ⚠️ **opengraph-image.tsx를 새로 추가하면 next.config.ts `outputFileTracingIncludes`에 그 경로를 반드시 추가** — 빠뜨리면 로컬은 멀쩡하고 운영에서만 한글이 사라진다. 폰트 읽기 실패 시 브랜드만 남기고 이미지는 계속 나간다. ⚠️ 생성기는 조각(Fragment)으로 묶은 자식의 세로 배치를 잃는다 — 직속 자식으로 둘 것. 줄바꿈은 `
` 분할로 직접 그린다 |
| SEO 구조화 데이터(JSON-LD) ⭐신규 | [src/lib/jsonld.tsx](src/lib/jsonld.tsx) — `JsonLd`(`<script type="application/ld+json">`, `<` 이스케이프로 태그 끊김·XSS 차단) + 스키마 조립기 `organizationJsonLd`·`websiteJsonLd`·`productJsonLd`(가격 1개=Offer / 2개↑=AggregateOffer, KRW 정수 그대로)·`faqJsonLd`·`breadcrumbJsonLd`·`articleJsonLd`. 빈 값 키는 `compact()`가 제거. 적용: [홈](src/app/page.tsx)(Organization+WebSite, `front_settings`의 `company_*`·`site_name`·`support_email` 재사용) · [상품 상세](src/app/product/[slug]/page.tsx)(Product+SoftwareApplication·Breadcrumb·상품 FAQ) · [FAQ](src/app/faq/page.tsx)(FAQPage) · [블로그 상세](src/app/blog/[slug]/page.tsx)(BlogPosting+Breadcrumb). 리치 HTML 설명은 `richToPlainText`로 평문화 후 주입 |
