  ...
  return (
    <div className="w-full max-w-md">
      ...
      <div className="text-center mb-10">
        <div className="w-14 h-14 rounded-lg bg-pen/5 border border-pen/40 flex items-center justify-center mx-auto mb-5">
          <Key size={24} className="text-pen" />
        </div>
        <h1 className="text-2xl font-serif font-black text-ink mb-2">라이선스 활성화</h1>
      ...
      <div className="bg-paper-raised border border-rule rounded-lg p-6 mb-4">
        ...
        <input
          ...
          maxLength={19}
          spellCheck={false}
          className="w-full bg-paper-raised border border-rule rounded-md px-4 py-3 text-center text-lg font-mono tracking-widest text-ink placeholder-ink-faint focus:outline-none focus:border-pen focus:ring-2 focus:ring-pen/15 transition-colors"
        />
        ...
        <button
          ...
          className="mt-4 w-full flex items-center justify-center gap-2 bg-pen text-white font-semibold py-3 rounded-md hover:bg-pen-dark disabled:opacity-40 disabled:cursor-not-allowed transition-all duration-200"
        >
          {isPending ? (
        ...
        >
          {result.found ? (
            <>
              <div className="flex items-center gap-2 mb-4">
                ...
                <span
                  className={`text-sm font-semibold ${
                    result.status === 'active' ? 'text-emerald-700' : 'text-amber-700'
                  }`}
                >
              ...
              {result.isOwner ? (
                <Link
                  ...
                  className="mt-5 flex items-center justify-center gap-2 text-sm text-pen hover:text-pen-dark transition-colors"
                >
                ...
                <p className="mt-4 text-xs text-ink-faint text-center">
                  ...
                  <Link href="/auth/login" className="text-pen hover:text-pen-dark hover:underline">
                    로그인
                  </Link>
                  하여 내 라이선스를 관리하세요.
                </p>
            ...
            <div className="flex items-start gap-2">
              <XCircle size={18} className="text-seal shrink-0 mt-0.5" />
              <div>
                <p className="text-sm font-semibold text-seal mb-1">라이선스를 찾을 수 없습니다</p>
                <p className="text-xs text-ink-soft">{result.error}</p>
              </div>
            </div>
          )}
        </div>
      ...
      {/* 하단 링크 */}
      <div className="mt-6 text-center text-xs text-ink-faint space-x-4">
        ...
        <span>·</span>
        <Link href="/dashboard/support" className="hover:text-ink transition-colors">
          고객지원
        </Link>
      </div>
    </div>
