...
import { type OrgLicenseInput, validateInput } from './_lib/orgLicenseSql'
...
export async function issueOrgLicense(input: OrgLicenseInput): Promise<AdminActionResult<IssueResult>> {
  ...
  if (issueErr) {
    if ((issueErr as { code?: string }).code === '23505') {
      return { status: 'failed', reason: '이미 등록된 라이선스 키입니다(동시 발급 감지). 아무것도 만들어지지 않았습니다.' }
    }
    console.error('[org-license] 발급 실패:', issueErr.message)
    return { status: 'failed', reason: '발급에 실패했습니다. 아무것도 만들어지지 않았습니다(두 표는 한 묶음). 잠시 후 다시 시도해 주세요.' }
  }
  ...
  const result: IssueResult = { tier: null, expiresAt: null, orgName: null, pcCount: null }
  try {
