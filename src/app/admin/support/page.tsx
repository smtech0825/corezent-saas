...
export default async function SupportPage({
  ...
  let query = adminClient
    ...
    .select('id, user_id, subject, status, priority, is_read, created_at, updated_at')
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-[#94A3B8] mt-1">{list.length} tickets</p>
      </div>
        ...
        {list.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {list.map((t) => (
                  <tr key={t.id} className={`border-b border-[#1E293B]/50 hover:bg-[#0B1120]/40 transition-colors ${!t.is_read ? 'bg-amber-500/3' : ''}`}>
                    ...
                    <td className="px-4 py-3">
                      <span className={`font-medium ${!t.is_read ? 'text-white' : 'text-[#94A3B8]'}`}>
                        {!t.is_read && (
                          <span className="inline-block w-1.5 h-1.5 rounded-full bg-amber-400 mr-2 align-middle" />
                        )}
                        {t.subject}
                      </span>
                    </td>
