...
export const PROPERTY_RULE_GUIDES: Record<string, RuleGuide> = {
  ...
  [PROPERTY_RULE_KEYS.surtax]: {
    ...
    notes: [
      ...
      '두 세율 모두 0% 이하는 저장이 거부됩니다. 이 룰이 없으면 재산세 계산기가 제공되지 않습니다(부가 세목을 조용히 0으로 계산하지 않기 위해서입니다). 종합부동산세의 재산세 상당액 공제 계산은 본세만 쓰므로 이 룰이 없어도 동작합니다.',
      '세부담 상한은 본세에 적용되며, 지방교육세는 상한이 적용된 본세를 기준으로 계산됩니다. 도시지역분(과세표준 기준)에는 상한이 반영되지 않습니다.',
    ],
    skeleton: `{
  ...
  [PROPERTY_RULE_KEYS.burdenCap]: {
    ...
    notes: [
      '상한액 = 직전 연도 재산세액(본세) × capPercent ÷ 100. 상한은 본세에 적용되며, 지방교육세는 상한이 적용된 본세를 기준으로 계산됩니다. 도시지역분(과세표준 기준)에는 상한이 반영되지 않습니다.',
