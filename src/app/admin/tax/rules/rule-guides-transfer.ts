...
export const TRANSFER_RULE_GUIDES: Record<string, RuleGuide> = {
  ...
  [TRANSFER_RULE_KEYS.exemption]: {
    ...
    notes: [
      ...
      '⚠️ residenceIfAcquiredRegulated.appliesToAcquiredFrom (선택): 그 거주 요건이 “언제 취득한 주택부터” 적용되는지 — 부칙이 정한 적용 시작 취득일입니다. 그날 취득한 주택도 포함합니다(취득일 ≥ 이 날짜).',
      '적용 시작을 정한 부칙이 있는데 이 필드를 비우면, 그 전에 취득한 주택까지 거주 요건에 걸려 비과세에서 빠지고 세금이 실제보다 많게 계산됩니다. 반대로 그런 부칙이 없는 조문이라면 이 줄을 통째로 빼세요 — 그러면 취득일과 무관하게 적용합니다.',
