...
export const RULE_GUIDES: Record<string, RuleGuide> = {
  ...
  [COMMON_RULE_KEYS.regulatedHistoryFrom]: {
    title: '규제지역 이력 커버리지 시작일 — 조정대상지역 지정 이력이 이 날짜부터 빠짐없이 등록돼 있다는 선언입니다. 양도소득세의 “취득 당시 조정대상지역” 자동 판정이 이 룰로 켜집니다.',
    notes: [
      '세목을 반드시 "공통 (전 세목)"으로 두고 등록하세요. 특정 세목에 넣으면 계산기가 찾지 못해 자동 판정이 조용히 꺼집니다.',
      '이 룰을 등록하는 순간 자동 판정이 켜집니다. 규제지역 이력을 먼저 다 넣은 뒤 마지막에 등록하세요.',
      '날짜는 등록된 이력 중 가장 이른 지정일로 두세요. 실제보다 이른 날짜를 넣으면 이력이 없는 구간이 “그때 비규제였다”로 자동 판정되어 비과세 거주 요건이 빠지고 세금이 실제보다 적게 계산됩니다.',
      '위 “시행일”도 같은 날짜(또는 그보다 앞)로 두세요. 시행일을 늦게 잡으면 그보다 이른 양도일로 계산할 때 이 룰이 잡히지 않아, 화면은 자동 판정이 켜진 것처럼 보이는데 계산은 선택을 요구하는 어긋남이 생깁니다.',
      '이 날짜보다 이르게 취득한 경우에는 자동 판정하지 않고 사용자에게 직접 선택을 요청합니다(임의로 비규제로 보지 않습니다).',
      '룰이 없으면 자동 판정이 꺼진 채 사용자 선택으로만 계산됩니다 — 계산이 막히지는 않습니다.',
      '이력을 더 과거로 확장하면 이 날짜를 앞당기세요. 근거(법령명·조문·원문 링크)도 함께 입력합니다.',
    ],
    skeleton: `{
  "from": "«YYYY-MM-DD»"
}`,
