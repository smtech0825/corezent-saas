      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="relative sm:w-64">
          ...
          <input
            ...
            className="w-full bg-[#111A2E] border border-[#1E293B] rounded-xl pl-9 pr-8 py-2.5 text-sm text-white placeholder:text-[#94A3B8] focus:outline-none focus:border-[#38BDF8]/50 transition-colors"
          />
          {search && (
            <button
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[#1E293B]">
                  <th className="text-left px-5 py-3 text-xs text-[#94A3B8] font-medium">사용자</th>
                  <th className="text-left px-4 py-3 text-xs text-[#94A3B8] font-medium">이메일 / 상태</th>
                  <th className="text-left px-4 py-3 text-xs text-[#94A3B8] font-medium">역할</th>
                  <th className="text-left px-4 py-3 text-xs text-[#94A3B8] font-medium hidden md:table-cell">국가</th>
                  <th className="text-left px-4 py-3 text-xs text-[#94A3B8] font-medium hidden lg:table-cell">가입일</th>
                  <th className="text-left px-4 py-3 text-xs text-[#94A3B8] font-medium">역할 변경</th>
                  <th className="text-right px-4 py-3 text-xs text-[#94A3B8] font-medium">작업</th>
                </tr>
              </thead>
                    ...
                    <tr className="border-b border-[#1E293B]/50 hover:bg-[#0B1120]/40 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-col gap-1.5">
                          <span className="text-[#E2E8F0] text-xs truncate max-w-[160px]">{u.email}</span>
                      ...
                      {/* 가입일 */}
                      <td className="px-4 py-3 text-[#94A3B8] whitespace-nowrap hidden lg:table-cell">
                    ...
                    {expandedId === u.id && (
                      <tr className="border-b border-[#1E293B]/50">
                        <td colSpan={7} className="px-5 py-4 bg-slate-900/50">
                          ...
                          {u.orders.length === 0 ? (
                            <p className="text-sm text-[#94A3B8] py-4 text-center">구매 내역이 없습니다.</p>
                          ) : (
                              ...
                              <table className="w-full text-xs">
                                <thead className="sticky top-0 z-10 bg-[#0B1120]">
                                  <tr className="border-b border-[#1E293B]/40">
                                    <th className="text-left px-4 py-2 text-[#94A3B8] font-medium">주문 ID</th>
                                    <th className="text-left px-4 py-2 text-[#94A3B8] font-medium">금액</th>
                                    <th className="text-left px-4 py-2 text-[#94A3B8] font-medium">상태</th>
                                    <th className="text-left px-4 py-2 text-[#94A3B8] font-medium">날짜</th>
                                  </tr>
                                </thead>
                                <tbody>
                                  {u.orders.map((o) => (
                                    <Fragment key={o.id}>
                                      <tr className="border-b border-[#1E293B]/20 last:border-0 hover:bg-[#111A2E]/60">
                                        ...
                                        <td className="px-4 py-2.5">
                                          <span className={`font-semibold ${orderStatusStyle[o.status] ?? 'text-[#E2E8F0]'}`}>
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              disabled={page === 1}
              className="w-9 h-9 flex items-center justify-center text-[#E2E8F0] border border-[#1E293B] rounded-lg disabled:opacity-30 hover:bg-[#1E293B] transition-colors"
            >
              <ChevronLeft size={14} />
            </button>
            {pageNums.map((n) => (
              <button
                ...
                className={`w-9 h-9 text-xs font-medium rounded-lg transition-colors ${
                  n === page
                    ...
                    : 'text-[#E2E8F0] border border-[#1E293B] hover:bg-[#1E293B]'
                }`}
              >
                {n}
              </button>
          ...
          <div className="relative bg-[#111A2E] border border-[#1E293B] rounded-2xl p-6 w-full max-w-sm shadow-2xl">
            ...
            <div className="flex items-start gap-4 mb-4">
              ...
              <div>
                ...
                <p className="text-xs text-[#94A3B8] mt-0.5 break-all">{withdrawTarget.email}</p>
              </div>
            </div>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 px-4 py-2.5 text-sm text-[#E2E8F0] border border-[#1E293B] rounded-xl hover:bg-[#1E293B] transition-colors disabled:opacity-50"
              >
                취소
              </button>
              <button
