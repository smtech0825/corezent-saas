        ...
        <main className="flex-1">
          <section className="relative pt-20 pb-24 px-6">
            <div className="relative z-10 max-w-2xl mx-auto text-center">
              <div className="w-16 h-16 rounded-full bg-ok-soft border border-ok/20 flex items-center justify-center mx-auto mb-6">
                <CheckCircle2 size={32} className="text-ok" />
              </div>
              <h1 className="text-3xl font-bold text-ink font-serif mb-3">결제가 완료되었습니다</h1>
              ...
              {/* 영수증 요약 */}
              {order ? (
                <div className="text-left border border-rule bg-paper-raised rounded-card p-6 mb-10">
                  <h2 className="text-sm font-semibold text-ink mb-4">주문 요약</h2>
                  ...
                  <div className="pt-3">
                    <Link href="/dashboard/billing" className="text-sm text-mark hover:underline">전체 주문 내역 보기 →</Link>
                  </div>
                </div>
              ) : (
                <div className="text-left border border-rule bg-paper-raised rounded-card p-6 mb-10">
                  <p className="text-sm text-ink-soft">
                    주문 내역은 잠시 후 <Link href="/dashboard/billing" className="text-mark hover:underline">결제 내역</Link>에서 확인하실 수 있습니다.
                    {!user && <> 먼저 <Link href="/auth/login" className="text-mark hover:underline">로그인</Link>해 주세요.</>}
                  </p>
                </div>
              )}
              ...
              <div className="text-left">
                <h2 className="text-sm font-semibold text-ink mb-4">다음 단계</h2>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <NextStep href="/dashboard/licenses" icon={<Download size={18} className="text-mark" />} title="앱 다운로드" desc="내 라이선스에서 설치파일을 받으세요." />
                  <NextStep href="/activate" icon={<KeyRound size={18} className="text-mark" />} title="라이선스 인증" desc="앱에 라이선스 키를 입력해 인증하세요." />
                  <NextStep href="/dashboard" icon={<LayoutDashboard size={18} className="text-mark" />} title="시작하기" desc="대시보드에서 온보딩을 확인하세요." />
                </div>
              </div>
            </div>
          </section>
...
function Row({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="grid grid-cols-[80px_1fr] gap-3 py-2 border-b border-rule/60 last:border-0">
...
function NextStep({ href, icon, title, desc }: { href: string; icon: React.ReactNode; title: string; desc: string }) {
  return (
    <Link href={href} className="block border border-rule bg-paper-raised hover:border-mark/30 rounded-card p-5 transition-colors">
      <div className="w-9 h-9 rounded-lg bg-paper-shade border border-rule flex items-center justify-center mb-3">{icon}</div>
