...
export default async function PublicSectorPage() {
  ...
  return (
    <div className="theme-paper min-h-screen bg-paper text-ink flex flex-col">
      <PageBreadcrumb trail={[{ name: '기관 도입', path: '/public-sector' }]} />
      <Navbar />
        ...
        {hasProcurement && (
          <Section tone="shade" width="text" className="py-10 sm:py-12">
            ...
            <div className="mt-4 space-y-1.5 text-base text-ink-soft">
              ...
              {/* 판매 채널만 적는다 — 계약 방법(수의계약)은 아래 「수의계약 근거 법령」 칸이 담당한다.
                  벤처나라는 상품 판매 등록이 끝난 판매처라 빠지면 안 된다. */}
              <p>판매 채널: corezent.com 직접 구매 · 벤처나라(바로주문 2천만원·초과는 견적주문)</p>
            </div>
          </Section>
        )}
