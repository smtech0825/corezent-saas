 ...
 *        ⚠️ 금액 계산 규칙(보고됨): 공급가액 = 단가(product_prices.price 원) × 수량,
 *        부가세 = 공급가액의 10% 반올림, 합계 = 공급가액 + 부가세. 이 외의 계산은 없다.
...
// 한글 폰트 등록 — 모듈 로드 시 1회. 파일이 없으면 여기서 throw → 호출부가 발급을 막는다.
Font.register({
  family: 'NanumGothic',
  fonts: [
...
/** 원 단위 금액 표시 — 문서 서식 전용(사이트 화면은 formatKRW(cents)를 사용, 여기는 원 입력) */
  ...
  return (
    <Document title={`견적서 ${data.quoteNo}`} author={sp.name}>
      <Page size="A4" style={S.page}>
        ...
        {/* 품목 — 단가는 상품 가격(product_prices.price 원) 그대로 */}
        ...
        <View style={S.table}>
          <View style={S.rowFirst}>
            ...
            <Text style={[S.th, { flex: 2 }, S.center]}>단가</Text>
            <Text style={[S.th, { flex: 2 }, S.center, S.last]}>금액</Text>
          </View>
          <View style={S.row}>
            ...
            <Text style={[S.td, { flex: 2 }, S.right, S.last]}>{won(data.amounts.supply)}</Text>
          </View>
        </View>
