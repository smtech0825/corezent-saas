 ...
 * @매개변수: when - 조건 집합 / context - 엔진이 만든 판정 컨텍스트 / ruleKey - 안내문용 룰 키
 * @반환값: { matched } 또는 실패
 */
export function matchConditions(
  ...
  ruleKey: string,
): { ok: true; matched: boolean } | TaxEngineFailure {
  for (const [field, cond] of Object.entries(when)) {
    ...
    if (c.eq !== undefined && v !== c.eq) return { ok: true, matched: false }
    if (c.min !== undefined) {
      if (!isNum(c.min)) return invalid(ruleKey, `조건 '${field}'의 min이 숫자가 아닙니다.`)
      if (typeof v !== 'number' || v < c.min) return { ok: true, matched: false }
    }
    if (c.max !== undefined) {
      if (!isNum(c.max)) return invalid(ruleKey, `조건 '${field}'의 max가 숫자가 아닙니다.`)
      if (typeof v !== 'number' || v > c.max) return { ok: true, matched: false }
    }
    if (c.in !== undefined) {
      if (!Array.isArray(c.in)) return invalid(ruleKey, `조건 '${field}'의 in이 배열이 아닙니다.`)
      if (!c.in.some((item) => item === v)) return { ok: true, matched: false }
    }
  }
...
export function selectRateRow(
  ...
  context: Record<string, Json>,
  ruleKey: string,
): { ok: true; row: RateTableRow } | TaxEngineFailure {
  ...
  if (matched.length === 0) {
    return engineFail(
      ...
      `룰('${ruleKey}')의 세율표에 입력 조건에 해당하는 행이 없습니다. 해당 조건의 세율이 등록될 때까지 이 계산은 제공되지 않습니다.`,
      ruleKey,
    )
  }
